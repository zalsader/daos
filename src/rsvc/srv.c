      ...
      size_t size, d_rank_list_t *replicas, void *arg, struct ds_rsvc **svcp)
{
	...
	if (mode == DS_RSVC_DICTATE)
		rc = rdb_dictate(svc->s_db_path, svc->s_db_uuid, &rsvc_rdb_cbs, svc, &svc->s_db);
	else if (mode == DS_RSVC_CREATE)
		rc = rdb_create(svc->s_db_path, svc->s_db_uuid, size, replicas,
				&rsvc_rdb_cbs, svc, &svc->s_db);
	else
		rc = rdb_start(svc->s_db_path, svc->s_db_uuid, &rsvc_rdb_cbs, svc, &svc->s_db);
	if (rc != 0)
		goto err_svc;
