...
pmfs_readdir(struct pmfs *pmfs, struct pmfs_obj *obj, uint32_t *nr, struct dirent *dirs)
{
	...
	rc = vos_client_obj_list_dkeys_sync(pmfs->coh, obj->oid, &nr_children,
					    &len, kds, enum_buf, pmfs->task_ring);
	if (rc)
		D_GOTO(out, rc = daos_der2errno(rc));
		...
		daos_off_t off)
{
	...
	do {
		compute_dkey(obj->chunk_size, offset, &num_records, &record_i, &dkey_val);
		...
		sgl.sg_iovs = NULL;
		rc = create_sgl(user_sgl, num_records, &iov_offset, &current_iov_nr, &sgl);
		if (rc) {
			return -ENOMEM;
		}
