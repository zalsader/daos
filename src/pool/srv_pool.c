		...
		bool create)
{
	...
	for (i = 0; i < prop->dpp_nr; i++) {
		...
		case DAOS_PROP_PO_SCRUB_PAD:
			d_iov_set(&value, &entry->dpe_val,
				  sizeof(entry->dpe_val));
			rc = rdb_tx_update(tx, kvs, &ds_pool_prop_scrub_pad,
					   &value);
			if (rc)
				return rc;
			break;
	       ...
	       daos_prop_t **prop_out)
{
	...
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_FREQ)
		nr++;
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_PAD)
		nr++;
	...
	*prop_out = prop;
	...
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_PAD) {
		d_iov_set(&value, &val, sizeof(val));
		rc = rdb_tx_lookup(tx, &svc->ps_root, &ds_pool_prop_scrub_pad,
				   &value);
		if (rc != 0)
			return rc;
		D_ASSERT(idx < nr);
		prop->dpp_entries[idx].dpe_type = DAOS_PROP_PO_SCRUB_PAD;
		prop->dpp_entries[idx].dpe_val = val;
		idx++;
	}
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_CRED) {
