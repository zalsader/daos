...
dv_path_update_from_indexes(struct ddb_ctx *ctx, struct dv_tree_path_builder *vt_path)
{
	...
	if (is_path_idx_set(vt_path->vtp_akey_idx)) {
		...
		rc = dv_get_akey(ctx->dc_coh, vt_path->vtp_path.vtp_oid, &vt_path->vtp_path.vtp_dkey,
				 vt_path->vtp_akey_idx, &vt_path->vtp_path.vtp_akey);
		if (!SUCCESS(rc))
			goto out;
		...
		struct vos_tree_handlers *handlers, void *handler_args)
{
	...
	if (uuid_is_null(path->vtp_cont)) {
		...
		return ddb_vos_iterate(&param, VOS_ITER_COUUID, false, &anchors, handle_iter_cb, &ctx);
	}
	...
	if (daos_oid_is_null(path->vtp_oid.id_pub)) {
		 ...
		 */
		rc = ddb_vos_iterate(&param, VOS_ITER_RECX, recursive, &anchors, handle_iter_cb, &ctx);
		if (!SUCCESS(rc)) {
			vos_cont_close(coh);

			return rc;
		}
		rc = ddb_vos_iterate(&param, VOS_ITER_SINGLE, recursive, &anchors, handle_iter_cb, &ctx);
		vos_cont_close(coh);
